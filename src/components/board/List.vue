...
export default {
  data() {
    ...
    return {
      body: '', // 리스트 페이지 데이터전송
      board_code: '001', // 게시판코드
      list: '', // 리스트 데이터
      no: '', // 게시판 숫자처리
      paging: '', // 페이징 데이터
      start_page: { start_page: '', end_page: '' }, // 시작페이지
      page: this.$route.query.page ? this.$route.query.page : 1,
      keyword: this.$route.query.keyword,
      paginavigation: function () {
        // 페이징 처리 for문 커스텀
        var pageNumber = [];
        var startPage = this.paging.start_page;
        var endPage = this.paging.end_page;
  ...
  methods: {
    fnGetList() {
      ...
      this.$axios
        .get('http://localhost:5000/api/board', { params: this.body })
        .then((res) => {
          if (res.data.success) {
